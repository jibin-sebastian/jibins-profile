...
export function CertificationsSection() {
  ...
  return <div>
    <motion.div initial={{
      ...
      <h2 className="text-4xl font-bold text-white mb-2">{content.certifications.title}</h2>
    ...
    <div className="relative max-w-4xl 2xl:max-w-5xl 3xl:max-w-6xl mx-auto">
      <div className="relative aspect-[4/3] bg-slate-800/40 backdrop-blur-sm border border-slate-700/50 rounded-2xl overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.img key={currentIndex} src={certifications[currentIndex]} alt={`${content.certifications.title} ${currentIndex + 1}`} initial={{
            opacity: 0,
            scale: 0.95
          }} animate={{
            opacity: 1,
